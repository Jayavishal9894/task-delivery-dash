...
type Filter = "all" | "active" | "delayed" | "delivered";
...
function TeamPage() {
  ...
  const { tasks, advance, remove, refresh: refreshTasks } =
  ...
  const visible = useMemo(() => {
    ...
    return tasks.filter((t) => {
      ...
      if (filter === "delayed")
        return stage < 3 && new Date(t.due_at).getTime() < now;
      return true;
    });
  }, [tasks, filter, assignee]);

  const delivered = tasks.filter((t) => teamStage(t) === 3).length;
  const delayed = tasks.filter(
    (t) => teamStage(t) < 3 && new Date(t.due_at).getTime() < Date.now(),
  ).length;
  ...
  const makeWorkspace = async () => {
    ...
    try {
      ...
      setNewName("");
      await refresh();
      toast.success("Workspace created — you're the manager");
    } catch (e) {
    ...
    return (
      <TeamShell memberships={[]} active={null} onSelect={() => {}}>
        <div className="bg-card border rounded-2xl p-6 shadow-sm text-center">
          ...
          <h1 className="text-xl font-bold">Create your team workspace</h1>
          <p className="text-sm text-muted-foreground mt-1 mb-4">
            You'll be the manager — invite teammates and start assigning
            deliveries.
          </p>
  ...
  return (
    <TeamShell memberships={memberships} active={active} onSelect={setActive}>
      <div className="flex items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground capitalize">
            You're a {active.role} · {members.length} member
            {members.length === 1 ? "" : "s"}
          </p>
        </div>
        {isManager && user && (
          <InviteMemberDialog workspaceId={active.workspace.id} managerId={user.id} />
        )}
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
        {(["all", "active", "delayed", "delivered"] as Filter[]).map((f) => (
          <button
            key={f}
            type="button"
            onClick={() => setFilter(f)}
        ...
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground">
            {isManager
              ...
              : "Nothing assigned to you in this workspace yet."}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {visible.map((t) => (
            <TeamTaskCard
              ...
              canDelete={isManager}
              canAdvance={isManager || t.assigned_to === user?.id}
